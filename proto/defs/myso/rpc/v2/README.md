...
- `x-myso-chain-id`: Chain Id of the current chain
- `x-myso-chain`: Human-readable name of the current chain
- `x-myso-checkpoint-height`: Current checkpoint height
- `x-myso-lowest-available-checkpoint`: Lowest available checkpoint for which
...
- `x-myso-lowest-available-checkpoint-objects`: Lowest available checkpoint for
...
- `x-myso-epoch`: Current epoch of the chain.
- `x-myso-timestamp-ms`: Current timestamp of the chain, represented as number
  of milliseconds from the Unix epoch.
- `x-myso-timestamp`: Current timestamp of the chain, encoded in the
