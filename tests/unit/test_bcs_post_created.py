...
def _option_some_address(byte_val: int) -> bytes:
    return b"\x01" + _addr(byte_val)
...
def _option_some_u64(value: int) -> bytes:
    return b"\x01" + value.to_bytes(8, "little")


def encode_post_created_with_organization(
    *,
    ...
    parts = [
        ...
        _option_none(),  # spt_id
        b"\x01",  # poc_redirection_kind
        _addr(2),  # actor_address
        _option_none(),  # sub_agent_id
        _option_none(),  # organization_id
        b"\x00",  # action_identity_class
    ]
    return b"".join(parts)
...
def test_decode_post_created_with_organization():
    raw = encode_post_created_with_organization(
